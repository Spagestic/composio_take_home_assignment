    ...
    async (
      ...
      try {
        ...
        const effectiveAuthMethods =
          verifyResult.revisedAuthMethods ?? findings.authMethods
        ...
        let effectiveBuildability =
          verifyResult.revisedBuildability ?? findings.buildability
        ...
        if (
          ...
          /connected app|external client app|spring ['’]?26|oauth/i.test(
            buildabilityNote
          ) &&
        ...
        const correctionsCount =
          ...
          (verifyResult.revisedBuildability &&
          effectiveBuildability !== findings.buildability
            ? 1
            : 0)
        ...
        await step.runMutation(internal.apps.updateResearchInternal, {
          ...
          blocker:
            findings.blocker ??
            (effectiveBuildability === "ready" &&
            /connected app|external client app/i.test(buildabilityNote)
...
export const startResearch = mutation({
  ...
  handler: async (ctx, args): Promise<string> => {
    ...
    await ctx.db.patch(existing._id, {
      ...
      workflowIds: [
        workflowId,
        ...existingIds.filter((id) => id !== workflowId),
      ].slice(0, 5),
    })
...
export const startUnresearched = mutation({
  args: {},
  returns: v.object({
    queued: v.number(),
    skipped: v.number(),
    queuedRanks: v.array(v.number()),
  }),
  handler: async (ctx) => {
    const apps = await ctx.db.query("apps").withIndex("by_rank").collect()
    const eligible = apps
      .filter((app) => (app.researchStatus ?? "not_started") === "not_started")
      .sort((a, b) => a.rank - b.rank)

    const queuedRanks: number[] = []

    for (const app of eligible) {
      // The mutation is serialized by Convex, so a second bulk request sees
      // this queued status and cannot create a duplicate active workflow.
      ...
      const existingIds =
        app.workflowIds ?? (app.workflowId ? [app.workflowId] : [])
      await ctx.db.patch(app._id, {
        workflowId,
        workflowIds: [
          workflowId,
      ...
      queuedRanks.push(app.rank)
    }

    return {
      queued: queuedRanks.length,
      skipped: apps.length - queuedRanks.length,
      queuedRanks,
    }
  },
})
